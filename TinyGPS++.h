...
public:
   ...
   const time_t age() const    { time_t rawtime; time (&rawtime); return valid ? rawtime - lastCommitTime : (time_t)ULONG_MAX; }
...
public:
   ...
   uint32_t value()           { updated = false; return gps_time; }
   uint8_t hour();
   uint8_t minute();
   uint8_t second();
   uint8_t centisecond();

   TinyGPSTime() : valid(false), updated(false), gps_time(0)
   {}

private:
   bool valid, updated;
   uint32_t gps_time, newTime;
   time_t lastCommitTime;
   void commit();
